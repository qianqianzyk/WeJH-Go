...
func DeleteTheme(c *gin.Context) {
	...
	_, themeType, isDarkMode, err := themeServices.GetThemeByID(data.ID)
	if err != nil {
		_ = c.AbortWithError(200, apiException.ServerError)
		return
	}
...
func AddThemePermission(c *gin.Context) {
	...
	_, themeType, _, err := themeServices.GetThemeByID(data.ThemeID)
	if err != nil {
		_ = c.AbortWithError(200, apiException.ServerError)
		return
	}
...
func GetThemePermission(c *gin.Context) {
	...
	themeNames, err := themeServices.GetThemeNameByID(data.StudentID)
	if err != nil {
		_ = c.AbortWithError(200, apiException.ServerError)
		return
	}
